...
const mathBlocks: Record<string, string> = {
  'math-selftest': String.raw`\mathcal{L}(\theta) = \frac{1}{2}\, J_{\text{diss}}\, \Delta t \;-\; \frac{\sigma^2}{2}\, J_{\text{diff}}\, \Delta t \;+\; \delta E_f`,

  'math-sde': String.raw`dX_t^i = \left(-\nabla V(X_t^i) - \frac{1}{N}\sum_{j \neq i} \nabla\Phi(X_t^i - X_t^j)\right) dt + \sigma\, dW_t^i`,

  'math-ito': String.raw`\underbrace{\delta E_f}_{\text{free-energy change}} \;=\; \underbrace{\frac{\sigma^2}{2}\, J_{\text{diff}}\, \Delta t}_{\text{diffusion}} \;-\; \underbrace{J_{\text{diss}}\, \Delta t}_{\text{dissipation}} \;+\; \text{martingale}`,
